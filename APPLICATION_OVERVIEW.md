...
This Generate → Critic → Fixer loop can run up to 2 times automatically to ensure quality.
...
If you want even more refinement, you can click "Request Review" to run an additional quality analysis. The Test Case Reviewer agent will:
...
If you like the suggestions from the review, you can click "Apply Improvements" and the system will:
...
You can then view the improved test cases and repeat this review/improve cycle if needed.
...
To save costs and speed things up, the system caches LLM responses in `.cache/llm/`. If you analyze the same Epic twice, the second time is nearly instant because it uses cached responses.

The cache key is a hash of the prompt content, so if anything changes, it regenerates.
...
Blocks SQL injection attempts, XSS attacks, path traversal attempts, and command injection patterns.

### Phase 2.1: Image Security
All uploaded images are blocked before processing to prevent exposure of sensitive UI mockups. Only metadata (filename, size) is recorded.

### Phase 2.2: PII Detection
Uses Microsoft's Presidio library to detect 50+ types of personally identifiable information:
...
- Digital: IP addresses, usernames, URLs with PII

When PII is detected, it's replaced with synthetic identifiers that preserve format but remove sensitive data. The mapping is stored so it can be reversed if needed.
